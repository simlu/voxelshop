...
import com.vitco.app.low.triangulate.Grid2TriPolySlow;
...
public class Grid2PolyHelperTest {
    ...
    // test finding potential t-junction problem spots (in 3D)
    @Test
    public void testFindProblemSpots() throws IOException {
        BufferedImage imgIn = ImageIO.read(new File("C:\\Users\\flux\\Dropbox\\java\\VoxelShop\\Test Files\\grid2poly\\file1.png"));
        boolean[][] data = new boolean[imgIn.getWidth()][imgIn.getHeight()];
        for (int x = 0; x < imgIn.getWidth(); x++) {
            for (int y = 0; y < imgIn.getHeight(); y++) {
        ...
        // print potential problem spots
        for (short[][] poly : polys) {
            // poly and outlines
            for (short[] outline : poly) {
                for (int i = 0, len = outline.length - 2; i < len; i += 2) {
                    if (outline[i + 2] == outline[i]) {
                        // x are equal
                        int step = outline[i + 3] > outline[i + 1] ? 1 : -1;
                        for (int y = outline[i + 1] + step; y != outline[i + 3]; y += step) {
                            int x = outline[i] + (step == 1 ? -1 : 0);
                            gr.setColor(Color.ORANGE);
                            gr.setStroke(new BasicStroke(5f));
                            gr.drawLine(outline[i] * zoom, y * zoom, outline[i] * zoom - step * 10, y * zoom);
                            gr.setColor(Color.BLACK);
                            gr.setStroke(new BasicStroke(1f));
                            gr.drawRect((x) * zoom, (y-1) * zoom, zoom, 2*zoom);
                        }
                    } else {
                        // y are equal
                        int step = outline[i + 2] > outline[i] ? 1 : -1;
                        for (int x = outline[i] + step; x != outline[i + 2]; x += step) {
                            int y = outline[i + 1] + (step == -1 ? -1 : 0);
                            gr.setColor(Color.ORANGE);
                            gr.setStroke(new BasicStroke(5f));
                            gr.drawLine(x * zoom, outline[i + 1] * zoom, x * zoom, outline[i + 1] * zoom + step * 10);
                            gr.setColor(Color.BLACK);
                            gr.setStroke(new BasicStroke(1f));
                            gr.drawRect((x - 1) * zoom, (y) * zoom, 2 * zoom, zoom);
                        }
                    }
                }
            }
