...
public class ConsoleView extends ViewPrototype implements ConsoleViewInterface {
    ...
    public JComponent buildConsole(final FrameLinkagePrototype frame) {
        ...
        ArrayList<String> consoleData = console.getConsoleData(Console.LINE_BUFFER_COUNT);
        for (String line : consoleData) {
          textArea.append(line);
        }
        textArea.setEditable(false); // hide the caret
        // enable scrolling for this textarea
        ...
        console.addConsoleListener(new ConsoleListener() {
            ...
            public void lineAdded(String line) {
                // show if necessary
                if (scrollPane.autoShow && (!frame.isVisible() || // not visisble
                ...
                // add line
                textArea.append(line);
                // make sure there are not too many lines
        ...
        menuGenerator.buildMenuFromXML(menuPanel, "com/vitco/logic/frames/console/toolbar.xml");
        panel.add(menuPanel, BorderLayout.WEST);

        // register actions (auto show / auto scroll)
