...
public class TextureTools {
    ...
    public static int[] getOffsets(int width, int height, boolean checkHeight, HashMap<Point, int[]> pixels) {
        ...
        if (checkHeight) {
            ...
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    int[] pixel = pixels.get(new Point(x, y));
                    if (pixel != null) {
                        if (scanline[x] == null) {
                            scanline[x] = pixel[2];
    ...
    public static int[] compress(int width, int height, int front, int back, boolean useHeight, HashMap<Point, int[]> pixels) {
        int factor = 1;
        // compute inside dist
        int dist = back - front;
        if (dist > 1) {
                        ...
                        for (int y = 0; y < d; y++) {
                            ...
                            for (int x = 0; x < width; x++) {
                                ...
                                for (int i = 0; i < stepSize; i++) {
                                    ...
                                    Point p = new Point(x, front + y * stepSize + i);
                                    // obtain the pixel
                                    ...
                                    if (pixel != null) {
                                        ...
                                        if (lastColor == null) {
                                            lastColor = pixel[2];
                                            result.put(new Point(x, front + y), new int[]{x, front + y, pixel[2]});
                                        } else {
                        ...
                        for (int x = 0; x < width; x++) {
                            ...
                            for (int y = back; y < height; y++) {
                                ...
                                if (pixel != null) {
                                    ...
                                    result.put(new Point(newPixel[0], newPixel[1]), newPixel);
                                }
                            }
                        }
                        pixels.clear();
                    ...
                    if (dist % d == 0) {
                        ...
                        for (int x = 0; x < d; x++) {
                            ...
                            for (int y = 0; y < height; y++) {
                                ...
                                for (int i = 0; i < stepSize; i++) {
                                    // compute the current point
                                    Point p = new Point(front + x * stepSize + i, y);
                                    // obtain the pixel
                                    int[] pixel = pixels.get(p);
                                    if (pixel != null) {
                                        // check if the pixel color is consistent through this step
                                        if (lastColor == null) {
                                            lastColor = pixel[2];
                                            result.put(new Point(front + x, y), new int[]{front + x, y, pixel[2]});
                                        } else {
                                            if (lastColor != pixel[2]) {
                                                break loop;
                                            }
